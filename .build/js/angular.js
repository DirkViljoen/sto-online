...
myModule.controller('rooms-ctrl', function($scope, $http, $window, $interval, socket) {
    ...
    getdoctors = function(){
      $http.get('/api/doctors').then(function(response) {
        ...
        if (response.data) {
          ...
          $scope.doctors.splice(0,0,{'_id':'000000000000000000000000','name':'none','colour':'#fdffff'});
    ...
    getTherapists = function (){
      $http.get('/api/therapists').then(function(response) {
        ...
        if (response.data) {
          ...
          $scope.therapists.splice(0,0,{'_id':'000000000000000000000000','name':'none','colour':'#fdffff'});
  ...
  // helper functions
    ...
    buildrooms = function() {
      if ($scope.working == 3){
        ...
        console.log('updateDoneOrder start');
        updateDoneOrder();
      }
    };
  ...
  // timer specific
    ...
    function updateDoneOrder() {
      var lessthan;
      for (var i = 0; i < $scope.rooms.length; i++) {
        if ($scope.rooms[i].multipleBeds == 0) {
          lessthan = 0;
          for (var k = 0; k < $scope.rooms.length; k++) {
            if ((k != i) && ($scope.rooms[k].multipleBeds == 0) && ($scope.rooms[i].beds[0].doctor == $scope.rooms[k].beds[0].doctor) && (moment($scope.rooms[i].beds[0].therapistStop) > moment($scope.rooms[k].beds[0].therapistStop)))
              lessthan++;

            if (k == $scope.rooms.length - 1){
              $scope.rooms[i].beds[0].torder = lessthan + 1;
              if (i == $scope.rooms.length - 2) {
                allowRefresh = true;
                $scope.loading = false;
              }
            }
    ...
    function updateTimes() {
      for (r = 0; r < $scope.rooms.length; r++){
        ...
        $scope.rooms[r].beds[0].dstatus = '';
        $scope.rooms[r].beds[0].dstatusclass = '';
        $scope.rooms[r].beds[0].tstatus = '';
        $scope.rooms[r].beds[0].tstatusclass = '';
        $scope.rooms[r].beds[0].sstatus = '';
        $scope.rooms[r].beds[0].sstatusclass = '';

        // Red
        // Doctor
        if (!$scope.rooms[r].beds[0].doctorStart) {
          if ($scope.rooms[r].dTimerType == 'SSR' || $scope.rooms[r].dTimerType == 'SDR') {
            $scope.rooms[r].beds[0].dstatus = 'NS';
            $scope.rooms[r].beds[0].dstatusclass = 'status_red';
          };
        }

        // Therapist
        if (!$scope.rooms[r].beds[0].therapistStart) {
          if ($scope.rooms[r].tTimerType == 'SSR' || $scope.rooms[r].tTimerType == 'SDR') {
            $scope.rooms[r].beds[0].tstatus = 'NS';
            $scope.rooms[r].beds[0].tstatusclass = 'status_red';
          };
        }
        // Sister
        if (!$scope.rooms[r].beds[0].sisterStart) {
          if ($scope.rooms[r].sTimerType == 'SSR' || $scope.rooms[r].sTimerType == 'SDR') {
            $scope.rooms[r].beds[0].sstatus = 'NS';
            $scope.rooms[r].beds[0].sstatusclass = 'status_red';
          };
        }

        // Yellow
        // Doctor
        if (!$scope.rooms[r].beds[0].doctorDone && $scope.rooms[r].beds[0].doctorStart){
          if ($scope.rooms[r].dTimerType == 'SDR') {
            $scope.rooms[r].beds[0].dstatusclass = 'status_yellow';
            $scope.rooms[r].beds[0].dstatus = 'Busy';
          }

          if ($scope.rooms[r].dTimerType == 'SSR') {
            $scope.rooms[r].beds[0].dstatusclass = 'status_yellow';

            var t = Math.abs(new moment().diff(moment($scope.rooms[r].beds[0].doctorTimeIn).add($scope.rooms[r].beds[0].doctorDuration, 'm')));

            $scope.rooms[r].beds[0].dstatus = intToTime(Math.floor(t/1000));

          }
        };

        // Therapist
        if (!$scope.rooms[r].beds[0].therapistDone && $scope.rooms[r].beds[0].therapistStart){
          if ($scope.rooms[r].tTimerType == 'SDR') {
            $scope.rooms[r].beds[0].tstatusclass = 'status_yellow';
            $scope.rooms[r].beds[0].tstatus = 'Busy';
          }

          if ($scope.rooms[r].tTimerType == 'SSR') {
            $scope.rooms[r].beds[0].tstatusclass = 'status_yellow';

            var t = Math.abs(new moment().diff(moment($scope.rooms[r].beds[0].therapistTimeIn).add($scope.rooms[r].beds[0].therapistDuration, 'm')));

            $scope.rooms[r].beds[0].tstatus = intToTime(Math.floor(t/1000));
          }
        };

        // Sister
        if (!$scope.rooms[r].beds[0].sisterDone && $scope.rooms[r].beds[0].sisterStart){
          if ($scope.rooms[r].sTimerType == 'SDR') {
            $scope.rooms[r].beds[0].sstatusclass = 'status_yellow';
            $scope.rooms[r].beds[0].sstatus = 'Busy';
          }

          if ($scope.rooms[r].sTimerType == 'SSR') {
            $scope.rooms[r].beds[0].sstatusclass = 'status_yellow';

            var t = Math.abs(new moment().diff(moment($scope.rooms[r].beds[0].sisterTimeIn).add($scope.rooms[r].beds[0].sisterDuration, 'm')));

            $scope.rooms[r].beds[0].sstatus = intToTime(Math.floor(t/1000));
          }
        };
        ...
        if ($scope.rooms[r].beds[0].doctorDone){
          $scope.rooms[r].beds[0].dstatusclass = 'status_green';
          $scope.rooms[r].beds[0].dstatus = 'Done';
        };

        // Therapist
        if ($scope.rooms[r].beds[0].therapistDone){
          $scope.rooms[r].beds[0].tstatusclass = 'status_green';
          $scope.rooms[r].beds[0].tstatus = 'Done';

          if (!$scope.rooms[r].beds[0].doctorStart)
            $scope.rooms[r].beds[0].tstatus += ' ' + $scope.rooms[r].beds[0].torder;
        };

        // Sister
        if ($scope.rooms[r].beds[0].sisterDone){
          $scope.rooms[r].beds[0].sstatusclass = 'status_green';
          $scope.rooms[r].beds[0].sstatus = 'Done';
        };
      };
    }
...
myModule.controller('settings-ctrl', function($scope, $http, $window, $interval, socket) {
  ...
  $scope.staticLookup.roles = ['admin','user'];
  ...
  // database
    ...
    $scope.saveUsers = function() {
      ...
      for (var n = 0; n < $scope.users.length; n++){
        if ($scope.users[n]._id){
          // put
          $http.put('/api/users/' + $scope.users[n]._id, $scope.users[n])
            .then(
              function(result){
                sum++;
                if (sum == total){
                  saveSuccess('Users updated successfully');
                }
              }
            )
        }
          ...
          //post
          $http.post('/api/users', $scope.users[n])
            .then(
              function(result){
                sum++;
                if (sum == total){
      ...
      //Delete users
      if ($scope.users_old){
        for (var o = 0; o < $scope.users_old.length; o++){
          if ($scope.users_old[o]._id){
            // put
            $http.delete('/api/users/' + $scope.users_old[o]._id, $scope.users_old[o])
              .then(
                function(result){
                  sum++;
                  if (sum == total){
  ...
  // Settings
    ...
    $scope.removeUser = function(doctor) {
      var i = $scope.users.indexOf(user);
      if (!$scope.users_old){
        $scope.users_old = [];
      }
      $scope.users_old.push($scope.users[i]);
